...
// Only show achievements that have a linked proof article
...
function AchievementCard({ item, delay }: { item: Achievement; delay: number }) {
  ...
  const cardInner = (
    <>
      ...
      <div
        ...
        style={{ background: "radial-gradient(ellipse at top, rgba(245,158,11,0.09) 0%, transparent 65%)" }}
      />
      ...
      {/* Card content — flex column so footer always sits at bottom */}
      <div className="relative flex flex-col flex-1 p-6" style={{ zIndex: 1 }}>
        {/* Top badges row */}
        <div className="flex flex-wrap items-center gap-2 mb-4" style={{ paddingRight: 32 }}>
          <span
            className="text-[11px] font-bold px-2.5 py-1 rounded-full"
            style={{ background: cat?.bg ?? "rgba(255,255,255,0.1)", color: cat?.color ?? "rgba(255,255,255,0.8)" }}
          >
            {item.category}
          </span>
          <span
        ...
        <div className="pt-4 mt-auto" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          {/* Baca Berita CTA */}
          <div
            className="flex items-center gap-2 pt-3 group-hover:gap-3 transition-all duration-300"
          ...
          >
            ...
            <ArrowRight
              size={13}
              className="-translate-x-1 group-hover:translate-x-0 opacity-0 group-hover:opacity-100 transition-all duration-300"
              style={{ color: "#F59E0B" }}
            />
          </div>
        </div>
  ...
  if (hasArticle) {
    return (
      // h-full so the Link fills the grid cell height
      <Link href={`/aktivitas/${item.articleId}`} className="no-underline flex flex-col h-full">
        <motion.div {...motionProps} style={{ ...motionProps.style, cursor: "pointer" }}>
          {cardInner}
        </motion.div>
      </Link>
    );
  ...
  return (
    <section className="py-20 md:py-28 relative overflow-hidden" style={{ background: "linear-gradient(160deg, #f9f7ef 0%, #ede8d4 100%)" }}>
      ...
      <div
        ...
        style={{ background: "radial-gradient(circle, rgba(13,42,26,0.06) 0%, transparent 65%)" }}
      />
      {/* Decorative radial glow bottom-left */}
      <div
        className="absolute -bottom-40 -left-40 w-96 h-96 rounded-full pointer-events-none"
        style={{ background: "radial-gradient(circle, rgba(153,27,27,0.05) 0%, transparent 65%)" }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative">
        ...
        >
          {statsList.map(({ icon: Icon, label, value, suffix }, i) => (
            <div
              ...
              style={{
                borderRight: i < statsList.length - 1 ? "1px solid rgba(255,255,255,0.07)" : "none",
                borderBottom: "none",
              }}
            >
              {/* Icon */}
              ...
              {/* Label */}
              <div className="text-[11px] font-medium" style={{ color: "rgba(255,255,255,0.48)" }}>
                {label}
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {featured.map((item, i) => (
            <AchievementCard key={item.id} item={item} delay={i * 0.1} />
          ))}
        </div>
