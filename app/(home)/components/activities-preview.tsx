...
export default function ActivitiesSection() {
  ...
  const preview = (aktivitas ?? []).filter((item) => item.type === "event").slice(0, 3);
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div>
            ...
            >
              Acara &amp; Kegiatan <span style={{ color: "var(--color-maroon-500)" }}>Terkini</span>
            </motion.h2>
          </div>
                  ...
                  <Card className="group h-full cursor-pointer rounded-3xl border-neutral-100 hover:border-[color-mix(in_srgb,var(--color-maroon-500)_30%,transparent)] shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 gap-0 py-0">
                    ...
                    <div className="relative h-52 overflow-hidden rounded-t-3xl">
                      ...
                      <div className="absolute top-3 left-3 flex gap-1.5 flex-wrap">
                        <Badge
                          className="text-[11px] backdrop-blur-sm rounded-lg px-2.5 py-1 h-auto"
                          style={{ background: "rgba(153,27,27,0.9)", color: "white", border: "none" }}
                        >
                          Kegiatan
                        </Badge>
                        {st && (
                          <Badge
                            className="text-[11px] rounded-lg px-2.5 py-1 h-auto"
                    ...
                    <CardContent className="px-5 pt-4 pb-0 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-3 text-[12px] text-neutral-400 mb-2.5">
                        <span className="flex items-center gap-1">
                          <Calendar size={12} />
                          {formatDateRangeId(item.date, item.endDate)}
                        </span>
                        {item.time && (
                          <span className="flex items-center gap-1">
                            <Clock size={12} />
